...
BASE_LR    = 0.0001
BATCH_SIZE = 32
LR         = BASE_LR * BATCH_SIZE   # linear scaling rule
EPOCHS     = 10
N_TRAIN    = 20_000
SEED       = 0
...
# ── JIT warm-up ────────────────────────────────────────────────────────────────
print(f"batch_size={BATCH_SIZE}  lr={LR}  (base={BASE_LR} × {BATCH_SIZE})", flush=True)
print("JIT warm-up...", flush=True)
for kind in KINDS:
    _net = build_network(0)
    _net.fit(X_tr[:BATCH_SIZE], Y_tr[:BATCH_SIZE], lr=LR,
             learning_kind=kind, batch_size=BATCH_SIZE)
...
for i, kind in enumerate(KINDS, 1):
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        net.fit(X_tr[perm], Y_tr[perm], lr=LR,
                learning_kind=kind, batch_size=BATCH_SIZE)
        acc = evaluate(net)
