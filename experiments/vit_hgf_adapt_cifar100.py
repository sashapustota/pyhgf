"""
ViT-B/16 hybrid HGF transformer — online block adaptation on Split-CIFAR-100.

Identical to vit_hgf_adapt.py but uses CIFAR-100 (10 tasks × 10 classes).
Frozen ViT-B/16 achieves ~70-75% on CIFAR-100 vs 93-98% on CIFAR-10, giving
real gradient signal and headroom for HGF adaptation.
...
Requires experiments/cifar100_vit_embeddings/ — run vit_cifar100_embed.py first.

Run with:
    python experiments/vit_hgf_adapt_cifar100.py [--step-size 1e-3] [--n-train 1000]
"""
...
CACHE_DIR = Path(__file__).parent / "cifar100_vit_embeddings"
...
N_CLASSES  = 100
BATCH_SIZE = 32
TASKS      = [tuple(range(i, i + 10)) for i in range(0, 100, 10)]  # 10 tasks × 10 classes
N_TASKS    = len(TASKS)
...
# ── Linear probe (768 → 100) ──────────────────────────────────────────────────

def train_probe_from_cache(n_epochs=20):
    if not CACHE_DIR.exists():
        raise FileNotFoundError(
            f"Embeddings not found at {CACHE_DIR}. Run vit_cifar100_embed.py first."
        )
...
def precompute_cache(vit, hgf_blocks, dataset, batch_size=64, desc=""):
    ...
    for i, (imgs, labels) in enumerate(loader):
        ...
        all_tokens_cls.append(tokens[:, 0].cpu().float().numpy())
        all_h11.append(h.cpu().float().numpy())
        all_labels.append(labels.numpy())
    print(f"\r  Caching {desc}: done ({n} batches)          ")
    return (
        np.concatenate(all_tokens_cls, axis=0),
        np.concatenate(all_h11,        axis=0),
...
def embed_block11(vit, hgf_block11, tokens_cls, h11):
    B, N, D = h11.shape
    delta_np  = np.asarray(hgf_block11.predict(h11.reshape(B * N, D)))
    delta_cls = delta_np.reshape(B, N, D)[:, 0, :]
    cls_raw   = torch.from_numpy(tokens_cls + delta_cls).to(DEVICE, dtype=torch.float32)
    with torch.no_grad():
        cls_embed = vit.encoder.ln(cls_raw)
...
def hgf_adapt_step_cached(vit, hgf_block11, probe, tokens_cls, h11, labels, step_size):
    ...
    delta_np = np.asarray(hgf_block11.predict(h_flat))
    delta_cls_np = delta_np.reshape(B, N, D)[:, 0, :]
    ...
    grad_cls = delta_t.grad.detach().cpu().float().numpy()

    grad_full        = np.zeros_like(delta_np)
    grad_full[::N]   = grad_cls
...
def run_hgf_adapt(vit, hgf_block11, probe, train_splits, test_splits, step_size, rng):
    ...
    for t, (tr_cls, tr_h11, tr_labels) in enumerate(train_splits):
        _reset_hgf_layer_states(hgf_block11)
...
def main():
    ...
    print("\nTraining linear probe on cached CIFAR-100 ViT embeddings...")
    probe = train_probe_from_cache(n_epochs=20)

    DATA_DIR.mkdir(parents=True, exist_ok=True)
    train_ds = datasets.CIFAR100(DATA_DIR, train=True,  download=True, transform=_TRANSFORM)
    test_ds  = datasets.CIFAR100(DATA_DIR, train=False, download=True, transform=_TRANSFORM)
    ...
    print(f"\nTasks: 10 × 10 classes | n_train={args.n_train} | n_eval={args.n_eval}")
    ...
    results_path = RESULTS_DIR / "vit_hgf_adapt_cifar100.csv"
    ...
    for ax, (title, am, bwt) in zip(axes, conditions):
        for j in range(N_TASKS):
            ...
            ax.plot(x_ticks, vals, marker="o", label=f"T{j+1} (cls {j*10}-{j*10+9})")
    ...
    fig.suptitle("Split-CIFAR-100: frozen ViT-B/16 backbone, online block 11 adaptation")
    fig.tight_layout()
    out = Path(__file__).parent / "vit_hgf_adapt_cifar100.png"
